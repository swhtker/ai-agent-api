...
async def send_training_message(
    ...
    try:
        ...
        user_task = models.Task(
            name=f"Training: {request.message[:50]}",
...
async def get_training_history(
    ...
    try:
        tasks = (
            ...
            .filter(models.Task.task_type == "training")
        ...
        for task in tasks:
            metainfo = task.metainfo or {}
            role = metainfo.get("role", "user")
            timestamp = metainfo.get(
                "timestamp",
                task.created_at.isoformat() if getattr(task, "created_at", None) else datetime.utcnow().isoformat(),
            )

            messages.append(
                TrainingHistoryItem(
                    id=metainfo.get("message_id", str(task.id)),
                    role=role,
